...
template <typename T> size_t Partition(std::vector<T> &A, size_t low, size_t high, size_t &m, size_t &c) {
    T pivot = A[high];
...
template <typename T> void QuickSortHelper(std::vector<T> &A, size_t low, size_t high, size_t &m, size_t &c) {
...
template <typename T> size_t QuickSort(std::vector<T> &A, size_t len) {
    size_t m = 0, c = 0;
    QuickSortHelper(A, 0, len - 1, m, c);
    return m + c;
}

template <typename T> void Heapify(std::vector<T> &A, size_t len, size_t i, size_t &m, size_t &c) {
    size_t largest = i;
    size_t left = 2 * i + 1;
    size_t right = 2 * i + 2;
...
template <typename T> size_t HeapSort(std::vector<T> &A, size_t len) {
    size_t m = 0, c = 0;
