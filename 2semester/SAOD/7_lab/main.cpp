...
/**
 * @struct phoneInfo
 * @brief Structure representing phone information including first name, last
 * name, patronymic, and phone number.
 */
struct phoneInfo {
...
/**
 * @brief Compare function to compare two phoneInfo structures based on
 * specified keys.
...
bool comprasion(phoneInfo x, phoneInfo y, int &fKey, int &sKey, bool &reverse) {
  ...
  switch (fKey) {
    case 1: {
      fKeyPole1 = x.firstname;
      fKeyPole2 = y.firstname;
      break;
    }
    case 2: {
      fKeyPole1 = x.lastname;
      fKeyPole2 = y.lastname;
      break;
    }
    case 3: {
      fKeyPole1 = x.patronymic;
      fKeyPole2 = y.patronymic;
      break;
    }
    case 4: {
      fKeyPole1 = x.phoneNumber;
      fKeyPole2 = y.phoneNumber;
      break;
    }
  }
  switch (sKey) {
    case 1: {
      sKeyPole1 = x.firstname;
      sKeyPole2 = y.firstname;
      break;
    }
    case 2: {
      sKeyPole1 = x.lastname;
      sKeyPole2 = y.lastname;
      break;
    }
    case 3: {
      sKeyPole1 = x.patronymic;
      sKeyPole2 = y.patronymic;
      break;
    }
    case 4: {
      sKeyPole1 = x.phoneNumber;
      sKeyPole2 = y.phoneNumber;
      break;
    }
  }
  if (reverse) {
...
/**
 * @brief Perform selection sort on a vector of phoneInfo objects based on
 * specified keys.
 ...
 * @param reverse Flag indicating whether to sort in reverse order.
 */
void SelectSort(std::vector<phoneInfo> &phoneBook, int &fKey, int &sKey,
                bool &reverse) {
  int k;
...
void printTable(std::vector<phoneInfo> &phoneBook) {
  std::cout << std::left << std::setw(15) << "First name" << std::setw(15)
            << "Last name" << std::setw(15) << "patronymic" << std::setw(15)
            << "Phone number" << "\n\n";

  for (auto phoneInfo : phoneBook) {
    std::cout << std::left << std::setw(15) << phoneInfo.firstname
              << std::setw(15) << phoneInfo.lastname << std::setw(15)
              << phoneInfo.patronymic << std::setw(15) << phoneInfo.phoneNumber
              << "\n";
  }
  std::cout << "\n";
}
