 ...
 */
StudentApp::StudentApp(QWidget *parent) : QMainWindow(parent) {
    setupUi();
}
 /**
                ...
                if (!parts.isEmpty() && parts[0] == searchLastName) {
                    ...
                    if (dialog.exec() == QDialog::Accepted) {
                        ...
                        for (int j = 0; j < updatedStudent.subjects.size(); ++j) {
                            ...
                            QString updatedName = QInputDialog::getText(this, "Update Subject", "Update Subject Name " + QString::number(j + 1) + ":", QLineEdit::Normal, subject.name, &ok);
                            if (!ok) return;

                            int updatedGrade = QInputDialog::getInt(this, "Update Subject", "Update Grade for " + updatedName + ":", subject.grade, 0, 100, 1, &ok);
                            if (!ok) return;
 ...
 *
 * Эта функция добавляет запись о студенте в файл, включая фамилию, имя, отчество
 * и информацию о каждом предмете и соответствующих оценках.
 *
 ...
 *
 * Эта функция создает главное окно, настраивает компоновку и добавляет различные
